...
export const CONCEPTS: ConceptMeta[] = [
  {
    ...
    realWorld:
      "Every time you open Twitter, it doesn't re-query millions of tweets. Netflix pre-loads what you're likely to watch next. Amazon's product pages are cached — your DB would melt otherwise.",
    ...
    realWorldDetails:
      'Netflix caches popular shows at CDN edge locations so millions of user streams play without lag. When you browse, a show\'s metadata is served from the nearest cache—not Netflix\'s central servers.',
    processSteps: [
  ...
  //   what: "Instead of Service A calling Service B directly and waiting, A drops a message into a queue and moves on. B picks it up when ready. Kafka, RabbitMQ, SQS — async by design.",
  //   realWorld:
  //     "When you post on Instagram, the notification to your followers isn't sent synchronously. It's queued. Email services, payment processors, video encoding — all queue-based.",
  ...
  //   overview: 'Message queues decouple services by allowing asynchronous communication. This simulator demonstrates a FIFO queue with configurable producers and workers, backpressure handling, failure simulation, and retry logic.',
  //   howItWorks: 'Producers send messages to a queue at a configurable rate; workers pick them up and process them concurrently. Messages can fail, be retried, or go to a dead-letter queue when retries are exhausted.',
  //   realWorldDetails: 'Instagram queues notifications so posting doesn\'t block follower updates. When your post goes viral, millions of notifications are queued and processed gradually by worker pools.',
  //   processSteps: [
  //     { num: 1, desc: 'Service A produces a message' },
  //     { num: 2, desc: 'Message is stored in the queue' },
  //     { num: 3, desc: 'Service A returns immediately (decoupled)' },
  //     { num: 4, desc: 'Worker polls the queue for messages' },
  //     { num: 5, desc: 'Worker processes the message' },
  //     { num: 6, desc: 'Message marked as consumed or moved to DLQ on failure' },
  //   ],
  // },
  // {
  //   id: 'cdn',
  ...
  //   what: "JavaScript is single-threaded but non-blocking. The event loop constantly checks: is anything ready? Resolved promise? File read done? Timer fired? It handles it — without waiting.",
  //   realWorld:
  ...
  //   what: 'Instead of one giant codebase, you build small focused services that each own a domain. User service, payment service, notification service — each deployed and scaled separately.',
  //   realWorld:
